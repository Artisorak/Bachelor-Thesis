...
def animate(i):
    if achOutName == "blast_part_2": i *= 10
    t = 1.0 + dt*i
    print("")
    print("Processing frame /cluster/scratch/ariess/%s/%s.%s"%(achOutName, achOutName, str(i).zfill(5)))
    try:
    ...
    if blast_mode:
        ...
        if mode == "density":
            ...
            ax.scatter(gas['r']*L, gas['rho']*RHO, s=0.5, edgecolors="none", c=np.log10(gas['rho']))
        elif mode == "pressure":
            # plot particle x-position vs. pressure
            ax.scatter(gas['r']*L, gas['temp']*gas['rho']*kB/u*RHO, s=0.5, edgecolors="none", c=np.log10(gas['rho']))
        else:
            # plot particle x-position vs. density
            ax.scatter(gas['r']*L, gas['rho']*RHO, s=0.5, edgecolors="none", c=np.log10(gas['rho']))
        ...
        if mean:
            ...
            if mode == "density":
                ax.plot(np.linspace(dx/2, 0.5-dx/2, len(meanrho))*L, meanrho*RHO, 'r-', linewidth=0.75, label=r'mean density')
            elif mode == "pressure":
                ax.plot(np.linspace(dx/2, 0.5-dx/2, len(meanPr))*L, meanPr*kB/u*RHO, 'r-', linewidth=0.75, label=r'mean pressure')
            else:
                ax.plot(np.linspace(dx/2, 0.5-dx/2, len(meanrho))*L, meanrho*RHO, 'r-', linewidth=0.75, label=r'mean density')
        ...
        if mode == "density":
            ax.scatter(gas['x']*L, gas['rho']*RHO, s=0.5, edgecolors="none", c=np.log10(gas['rho']))
        elif mode == "pressure":
            ax.scatter(gas['x']*L, gas['temp']*gas['rho']*kB/u*RHO, s=0.5, edgecolors="none", c=np.log10(gas['rho']))
        else:
            ax.scatter(gas['x']*L, gas['rho']*RHO, s=0.5, edgecolors="none", c=np.log10(gas['rho']))
        ...
        if mean:
            ...
            if mode == "density":
                ax.plot(np.linspace(-0.5+dx/2, 0.5-dx/2, len(meanrho))*L, meanrho*RHO, 'r-', linewidth=0.75, label=r'mean density')
            elif mode == "pressure":
                ax.plot(np.linspace(-0.5+dx/2, 0.5-dx/2, len(meanPr))*L, meanPr*kB/u*RHO, 'r-', linewidth=0.75, label=r'mean pressure')
            else:
                ax.plot(np.linspace(-0.5+dx/2, 0.5-dx/2, len(meanrho))*L, meanrho*RHO, 'r-', linewidth=0.75, label=r'mean density')
        ...
        if "sod" in achOutName:
            ...
            if mode == "density":
                ax.plot(xgrid[0,:], rhoE[0,:], 'k-', linewidth=0.75, label=r'analytic density')
                print("Simulation:")
                print(meanrho*RHO)
                print("Analytic:")
                print(rhoE[0,:])
            elif mode == "pressure":
                ax.plot(xgrid[0,:], PrE[0,:], 'k-', linewidth=0.75, label=r'analytic pressure')
                print("Simulation:")
...
mp4writer = animation.FFMpegWriter(fps=nsteps/30.0)
anim.save('plots/plot_%s_%s.mp4'%(achOutName, mode), writer=mp4writer, dpi=300)